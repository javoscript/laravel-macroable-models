...
class MacroableModelsTest extends TestCase
{
    ...
    {
        $this->mm->addMacro(DummyModel::class, 'exampleMacro', function () {
            return true;
        });
        $this->model->exampleMacro();
    ...
    // Call a function on a dummy model after registering the macro first and then removing it throws a BadMethod exception
    public function testBadMethodCallExceptionAfterRemovingTheMacroButWithAnotherModelWithTheSameMacro()
    {
        $this->expectException(\BadMethodCallException::class);

        $this->mm->addMacro(DummyModel::class, 'exampleMacro', function () {
            return 'from dummy model';
        });

        $this->mm->addMacro(AnotherDummy::class, 'exampleMacro', function () {
            return 'from another dummy';
        });

        $this->assertEquals($this->model->exampleMacro(), "from dummy model");
        $this->assertEquals($this->anotherModel->exampleMacro(), "from another dummy");
    ...
    public function testMacrosForModelReturnsMultipleMacrosAfterRegisteringThemForTheModel()
    {
        ...
        $this->mm->addMacro(DummyModel::class, 'anotherMacro', function () {
            return 2;
        });
    ...
    {
        $this->mm->addMacro(DummyModel::class, 'exampleMacro', function ($a) {
            return 1 + $a;
        });
    ...
    {
        $this->mm->addMacro(DummyModel::class, 'exampleMacro', function ($name, $surname) {
